...
import { Product } from "../Screens/Component/cart/CartScreens";
...
const CartProvider = ({children}:any) => {
    ...
    const setCartStorageFromDetail = async (item:Product) => {
        let newItem = {...item, cartQuantity: 1, totalPrice: item.price}
        if(cart.length == 0){
    ...
    return(
        <CartItem.Provider value={{cart,setCartStorageFromCart, getCart, setCartStorageFromDetail, totalPrice, getTotalPrice, calcTotalPrice, order, getOrder, setOrderStorage, payment, getPayment, setPaymentStorage,delCart}}>
            {children}
        </CartItem.Provider>
    )
